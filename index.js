...
app.post("/set-profile", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ email, studentId }, JWT_SECRET, { expiresIn: "1h" });
...
app.get('/faculty/dashboard', authenticateToken, async (req, res) => {
  try {
    ...
    const completionRate = await getCompletionRateBySkill(facultyId);
  ...
  try {
    ...
    const [rows] = await db.query(`
      SELECT 
        ...
        r.conducted_time,
        r.conducted_date,
        s.skill_name -- Assuming a 'skills' table has 'skill_name'
      FROM Results r
...
app.get('/chatbot', (req, res) => {
  res.render('chatbot'); // make sure chatbot.ejs exists
});
...
// Endpoint to handle student results evaluation
app.post('/evaluateResults', authenticateToken, async (req, res) => {
  try {
    ...
    console.log("Student ID from database:", studentId);

    // Fetch results
    const [rows] = await db.query(`
      SELECT s.skill_name, r.skill_id, r.Result
      FROM results r
      JOIN skills s ON r.skill_id = s.skill_id
      WHERE r.student_id = ?
    `, [studentId]);
