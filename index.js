...
const authenticateToken = (req, res, next) => {
  ...
  // ✅ Prioritize token from cookies (more secure than headers)
  if (req.cookies && req.cookies.auth_token) {
      token = req.cookies.auth_token;
      console.log("✅ Token extracted from cookies:", token);
  } 
  // ✅ Fallback: If no cookie, check the Authorization header
...
app.get("/download-excel", (req, res) => {
  ...
  fs.access(filePath, fs.constants.F_OK, (err) => {
    if (err) {
      ...
      return res.status(404).send("❌ File not found. Please upload the file and try again.");
    }
    ...
    res.download(filePath, "Mapping_Format.xlsx", (err) => {
      if (err) {
        ...
        return res.status(500).send("❌ Error downloading file. Please try again.");
      }
    });
  });
});
...
app.get("/admindash", authenticateToken, (req, res) => {
  console.log("✅ Admin Access Check:", req.user);
...
app.post("/send-credentials", async (req, res) => {
  ...
  try {
      ...
      await db.promise().query(
        "DELETE FROM faculty_login WHERE email = ?",
        [email]
    );
...
app.delete("/delete-selected-faculty", async (req, res) => {
  ...
  try {
      ...
      // Step 1: Find Faculty IDs
      const [facultyRows] = await db.promise().query(
          "SELECT faculty_id FROM faculty WHERE email IN (?)",
          [emailList]
      );
      ...
      const facultyIds = facultyRows.map(row => row.faculty_id);

      // Step 2: Delete related records (handle foreign key constraints)
      await db.promise().query("DELETE FROM faculty_skill_mapping WHERE faculty_id IN (?)", [facultyIds]);
      await db.promise().query("DELETE FROM faculty_student_mapping WHERE faculty_id IN (?)", [facultyIds]);
      await db.promise().query("DELETE FROM faculty_login WHERE email IN (?)", [emailList]);

      // Step 3: Delete from faculty table
      await db.promise().query("DELETE FROM faculty WHERE faculty_id IN (?)", [facultyIds]);

      res.json({ success: true, message: "✅ Selected faculty records deleted successfully!" });
...
app.get("/result", async (req, res) => {
  try {
    ...
    let students = [];
    try {
      [students] = await db.promise().query("SELECT student_id FROM students");
    } catch (error) {
      console.error("❌ Error fetching student IDs:", error);
      return res.status(500).send("❌ Error fetching student IDs.");
    }
    ...
    if (studentId) {
      try {
        ...
        return res.status(500).send("❌ Error fetching student results.");
      }
    }
    ...
    console.error("❌ Error processing results request:", error);
    res.status(500).send("❌ Internal Server Error.");
  }
});
...
app.post('/delete-skill-viewmapping/:id', async (req, res) => {
  try {
    const id = req.params.id;
    ...
    res.status(500).json({ message: "❌ Failed to delete skill mapping" });
  }
});
